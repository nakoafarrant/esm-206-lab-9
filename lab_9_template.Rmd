...
# going to be problematic to include price per sq ft because that is calculated based on the output price that is desired. This will improve model fit in a bias way
...
# 53% of housing price is explained by this model (multiple R-squared)
# doesn't adjust for the fact that as you add more predictor variables, model fit will naturally increase

# adjusted R-squared account for the fact that model fit increases with more variables
```
...
homes_sub$Status <- factor(homes_sub$Status) # not overwriting any of the data, just telling R that when you look at this, recognize it as a factor
# this allows use of forcats to do some wrangling according to factor

# b. Check to ensure it's a factor now
# Use class(homes_sub$Status)

# c. Check levels:
# Use levels(homes_sub$Status)
# Want to reassign this so reference level is regular sale
...
# set this level first and then lists others in alpha order. Could reorder levels further by separating by commas
...
# a. Model diagnostics:
plot(homes_lm3)
# tend to only look at the first 2 plots

# don't let a few data points in a large data set change your assumptions like heterosedacicity 
# same for a few points in the residuals distribution
```
...
full_aic <- AIC(homes_lm1) # valuable for comparing multiple models
...
price_predict <- predict(homes_lm3, newdata = df_new, se.fit = T, interval = "confidence") # it's a numeric, not a data frame.

# b. Bind predictions to the data to make it actually useful:
predict_df <- data.frame(df_new, price_predict)
# fit.fit is model fit
...
predict_graph <- ggplot(predict_df, aes(x = SqFt, y = fit.fit)) +
  ...
  facet_wrap(~Status) +
  geom_point(data = homes_sub, aes(x = SqFt, y = Price), alpha = 0.5) # add data from a different data set
